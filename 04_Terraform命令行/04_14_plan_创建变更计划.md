...
# 3 注意点 
## 3.1 The terraform plan command does not update your state file

The terraform plan command does not update your state file. Instead, it generates an execution plan by comparing the desired state (defined in your Terraform configuration files) with the actual state (stored in the state file) of your infrastructure. 
The plan shows the changes that Terraform will make to the infrastructure without actually applying those changes. 

The state file is only updated when you run terraform apply, which applies the changes to your infrastructure and updates the state file to reflect the new state.