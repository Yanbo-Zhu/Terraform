...
我们在`aws_instance`中定义了类型为`file`的预置器，该预置器可以本机文件或文件夹拷贝到目标机器的指定路径下。我们在预置器内部定义了`connection`块，类型是`ssh`。我们对`connection`的`host`赋值`self.public_ip`，在这里`self`代表预置器所在的母块，也就是`aws_instance.web`，所以`self.public_ip`代表着`aws_instance.web.public_ip`，也就是创建出来的主机的公网ip。
